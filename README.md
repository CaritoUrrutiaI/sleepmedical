...
- Se quitó infraestructura de WordPress que solo tenía sentido con un backend en vivo: discovery de XML-RPC (`xmlrpc.php@rsd`), discovery de la REST API y oEmbed (`wp-json/`, 2MB de JSON congelado sin ningún destino real), y los beacons de telemetría de Jetpack (`stats.wp.com`, `_stq`) que mandaban tracking a la cuenta de Jetpack del sitio original en cada carga de página.
- Se quitaron elementos visibles que ya no funcionan sin backend: la sección "Valoraciones" (reseñas de producto, siempre vacías, con formulario que no puede recibir envíos) de las 22 fichas de producto, y toda la infraestructura de feeds RSS (`feed/`, `comments/feed/`, ~40 carpetas) — snapshots XML congelados en la fecha del mirror que nunca se actualizarían. No había botón de "Añadir al carrito" en ninguna página (todos los productos usan el flujo "Leer más"/contacto de WooCommerce en vez de compra directa), así que no había nada que quitar ahí.
